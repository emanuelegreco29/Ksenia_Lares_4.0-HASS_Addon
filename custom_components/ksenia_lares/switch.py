            ...
            try:
                ...
                for switch in switches:
                    ...
                    if switch_id not in discovered_switch_ids:
                        ...
                        new_entities.append(
                            KseniaSwitchEntity(
                                ws_manager, switch_id, name, switch, device_info, base_id
                            )
                        )
...
class KseniaZoneBypassSwitch(SwitchEntity):
    ...
    def __init__(self, ws_manager, zone_id, name, zone_data, device_info=None, base_id=None):
    ...
    async def _handle_realtime_update(self, data_list):
        ...
        for data in data_list:
            if str(data.get("ID")) == str(self.zone_id):
                _LOGGER.debug("[zone_bypass] Entity %s update: %s", self.zone_id, data)
